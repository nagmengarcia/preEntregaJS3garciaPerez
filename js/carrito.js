const renderCarritoFinal = () => {
    ...
    if (carritoRecuperado.length > 0) {
        ...
        for (const b of carritoRecuperado) {
            ...
            <button class="removeFromCart" id="id-${b.id}"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
...
const eliminadorDeProducto = (e) => {
    const a = obtenerProductosLS();
    const b = a.filter( i => i.id !== `id-${(e)}`);
    return b;
}


const quitarUnSoloProducto = (idHTML) => {
    let i = document.getElementById(idHTML) || false;

    if (i!== false){
        i.addEventListener('click', () => {
            const newCarrito = eliminadorDeProducto(idHTML);
            guardarCarritoLS(newCarrito);
            renderCarritoFinal();
            renderSpanCarrito();
        })
    }
